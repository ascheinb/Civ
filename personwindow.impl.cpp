...
PersonWindow::PersonWindow(Model& model_in) :
...
void PersonWindow::add_entry_r( const char* name, const char* reltype, const int f_to, const int f_of)
{
...
void PersonWindow::add_entry_m( const char* name, const int l_to)
{
...
void PersonWindow::fill_buffers(int pid)
{
    ...
    {
        int person_ind = model->p.id2ind[model->p.person[pid].rships[irship].person_id];
